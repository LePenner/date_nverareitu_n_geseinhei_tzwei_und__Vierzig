...
def niceAnswer(data, complaint):
    ...
    processed_complaint = complaintPorcessing(PATHS, complaint)

    ticket_instance = Ticket_db()
    try:
        ticket_instance.create_ticket(
            UUID, email, complaint, response.to_dict()["candidates"][0]["content"]["parts"][0]["text"], processed_complaint, data)
